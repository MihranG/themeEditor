...
import {setThemeToLocalStorage, removeThemeFromLocalStorage, getThemeFromLocalStorage} from "./helpers/localStorage";
...
const SimpleEditor = ()=> {
    ...
    const isThereError: boolean = useSelector(
        (state: RootState)=> Object.values(state.theme).some(
            value => Object.values(value.items).some(
                item=> item.content.error
            )
        )
    );
