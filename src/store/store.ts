...
export const teamReducerSlice = createSlice({
  ...
  reducers: {
    addThemeState(
      state: IThemeState,
      action: PayloadAction<{ newTheme: IThemeState }>
    ) {
    ...
    editValue(
      ...
      action: PayloadAction<
        IThemeActionPayloadMain & { values: IThemeItemValue[]; error?: boolean }
      >
    ) {
    ...
    setError(
      state: IThemeState,
      action: PayloadAction<IThemeActionPayloadMain & { error: boolean }>
    ) {
