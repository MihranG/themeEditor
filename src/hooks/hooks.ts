...
import {stylesService, extractMetricsAndPrepareForStateUpdate} from "../utils/utils";
    ...
    const changeInputValue = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        timeoutRef.current = setTimeout(()=>{
            ...
            if(metricUnit){
                ...
                dispatch( editMetrics({id: ids.id, parentId:  ids.parentId, metrics, error: false }) );
            }
            if(!parsedCssString.error && parsedCssString.valuesArray){
                const valuesArray = extractMetricsAndPrepareForStateUpdate(parsedCssString.valuesArray);
                dispatch(editValue({id: ids.id, parentId:  ids.parentId, values: valuesArray, error: false}));
            }else{
