...
describe('<SimpleEditor />', () => {
    ...
    it('renders correctly and adds content', ()=>{
        ...
        expect(
            container?.querySelector(
                "[data-testid='editor_title']")?.textContent
    ...
    it('saves and resets on clicks', ()=>{
        const setLocalStorageSpy = jest.spyOn(localStorageService,'setThemeToLocalStorage' );
        const deleteLocalStorageSpy = jest.spyOn(localStorageService,'removeThemeFromLocalStorage' );
        act(()=>{
            render(
                <Provider store={store}>
                    <SimpleEditor />
