...
const FieldRowComponent: React.FC<{
  stylePropId: string;
  parentId: string;
}> = ({ stylePropId, parentId }) => {
  const [isEditing, setIsEditing] = useState(false);
  const dispatch = useDispatch();
  const styleProperty: IThemeItem = useSelector(
    (state: RootState) => state.theme[parentId].items[stylePropId]
  );
  const [
    inputValue,
    setInputValue,
    stylesStrings,
  ] = useStyleGenerator(styleProperty.content, { id: stylePropId, parentId });
  ...
  return (
    <FormGroup
      className={
        isEditing ? "row__container row__container-open" : "row__container"
      }
    >
      <Row className="field__row">
        <Col md={7}>
          <Label className="field__description" onClick={toggle}>
            <div className="field__name">{styleProperty.name}:</div>
            <div className="field__brief">{stylesStrings.css}</div>
          </Label>
        </Col>
        <Col md={4}>
          <div className="field__reference">{`${parentId}.${stylePropId}`}</div>
        </Col>
        {isEditing && (
          <Col md={1}>
        ...
        <FormGroup row className="field__row">
          ...
          <Col md={2}>
            <Button
              aria-label="ok"
              onClick={toggle}
              disabled={styleProperty.content.error}
            >
              OK
            </Button>
          </Col>
